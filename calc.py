...
class Calc:
    ...
    def calc(self):
        ...
        t = self.t(self.it)
        self._a, self._b, self._c, self._f = self.buildMatrix(self.nx, self.xstep, self.tstep, t, self.sigma, self.y, 0, 0, self.f)
        y1 = TDMA(self._a, self._b, self._c, self._f)

        # Runge Rule
        tstep2 = self.tstep * .5

        self._a, self._b, self._c, self._f = self.buildMatrix(self.nx, self.xstep, tstep2, t, self.sigma, self.y, 0, 0, self.f)
        y2 = TDMA(self._a, self._b, self._c, self._f)

        y2.extend([.0, .0])
        t += tstep2
        self._a, self._b, self._c, self._f = self.buildMatrix(self.nx, self.xstep, tstep2, t, self.sigma, y2, 0, 0, self.f)
